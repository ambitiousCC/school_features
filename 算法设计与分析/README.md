...
基本要素：
* 最优子结构：问题的最优解包含了其子问题的最优解时，称该问题具有最优子结构性质。问题的最优子结构性质提供了该问题可以用动态规划算法求解。自底向上的方式递归的从子问题的最优解逐步构造出整个问题的最优解

* 重叠子问题：递归算法自顶向下求解问题时，每次产生的子问题并不总是新问题，有的子问题被反复计算多次。对每个子问题只计算一次，然后将结果保存到表格中，再次需要的时候就从表格中查找。
...
t_ji是作业i在机器j上面调度所需要的时间
f_ji是作业i在机器上完成调度所需要的时间，这里的f有点难以理解，个人理解如下：
...
0-1背包问题：可以使用动态规划、回溯法和分支限界法，动态规划不需要排序，其余都需要按照质量价值比排序
...
void Loading(int i)
{
	...
	if(cw+w[i]<=c) //x[i] = 1 表示选择装入
	{
		cw+=w[i];
		Loading(i+1);
		cw-=w[i];
	}
	Loading(i+1); //x[i] = 0 表示选择不装入
}
```
4. 最优子结构的性质：如果问题得最优解包含所含包含的子问题的解也是最优的，我们就称为该问题具有最优子结构性质
5. 贪心选择性质：只是做当前情况下的最优选择，并不考虑整体，具有局部最优性。
5. 求解时间复杂度
7. 合并排序，快速排序