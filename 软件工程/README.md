...
2. 抽象
3. 逐步求精
4. 信息隐藏：设计和确定模块时，使得一个模块内包含的信息（过程和数据）对于不需要这部分信息的模块来说是不可访问的，对外隐藏了实现的细节，只能通过接口了解。
...
1. 软件定义时期：最重要的阶段，确定软件系统要做什么，又称为系统分析，包括：
* 问题定义：定义解决的问题
* 可行性分析：能否完成
* 需求分析：用户需求

2. 软件开发阶段：具体设计和实现软件
* 系统设计：如何实现目标系统【概要设计】->具体实现系统【详细设计】
* 系统实现：
	* 编码
	* 单元测试
	* 综合测试：集成测试以及验收测试等并产生测试报告

3. 运行维护：**软件生命周期中持续时间最长的阶段**，根据软件中运行的问题对其修改从而能够持续的满足用户的需求
维护：软件维护（改正性维护，适应性维护，完善性维护，防御性维护）

#### 需求分析

需求问题根本原因：软件没有固定的形态，所解决问题的复杂性（系统与人之间的交互复杂性，设计不同类别的用户）；最困难的就是不知道开发什么，便写出详细的需求说明书，用户也可能不知道他的需求是什么
...
2. 原型模型：
> 原型：软件的一个早期可运行的版本，反映最终系统的部分重要功能和性能指标
...
基本原则：尽可能接近人类认识世界解决问题的方法与过程；描述问题的问题空间与实现揭发的解空间在结构上尽可能一致
...
数据测试：检查输入的数据、返回的结构、中间计算结构是否正确

状态测试：通过检测软件的不同状态来验证程序的逻辑流程（通过性测试、实效性测试）

（多个）模块->（多个）单元测试->集成测试->确认测试->系统测试->可交付软件
...
动态|数据覆盖和代码覆盖（语句覆盖、条件覆盖、分支覆盖）|验证程序的结果（数据测试和状态测试）

> 代码覆盖：程序状态以及程序流程，设法进入和退出每一个模块，执行每一行的代码，进入软件的每一条逻辑和决策分支
> 三种方法：
> 1. 语句覆盖：每个执行语句一次
> 2. 条件覆盖：每个测试分支一次
> 3. 分支覆盖：判断的每个条件一次
...
定义：产品交付后对其进行修改，纠正故障、改进性能和其他属性，使产品适应改变了环境的具体过程

目的：保证软件系统能够持续地与用户环境、数据处理操作相协调，最终使系统稳定运行。

软件可维护性：软件能够被正确理解、改正、适应和完善，以适应新的环境的难易程度，是软件产品的一个重要质量特性i，是软件开发阶段各个时期的关键指标
